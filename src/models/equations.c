...
// similar to 405 but saves reservoir active operation in a state
void model406(double t, \
		const double * const y_i, \
		unsigned int dim, \
		const double * const y_p, \
		unsigned short num_parents, \
		...
		double *ans)
{
        ...
        unsigned int STATE_SNOW = 5;
        unsigned int STATE_DAM_STORAGE=6;
        unsigned int STATE_OPENING_PERCENTAGE=7;
	    ...
	    double q = y_i[STATE_DISCHARGE];      //[m^3/s]
        double dam_storage =y_i[STATE_DAM_STORAGE] ;//m^3
        double opening_percentage_initial = y_i[STATE_OPENING_PERCENTAGE]; //[%]
                ...
                if(output_mode==2){
                    ...
                    for(ii=0;i<SIZE1-1;i++){
                        if(
                            ...
                            && 
                            dam_storage + dam_input <= (max_storage_pond * percentage_storage_bins[ii+1])
                        )
                        {
                            dam_outflow = rating_flow * opening_percentage_bins[ii];
                            ans[STATE_OPENING_PERCENTAGE] = opening_percentage_bins[ii] - opening_percentage_initial;
                        }
                    }
